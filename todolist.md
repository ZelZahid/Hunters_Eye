...
- [ ] **Validate potion drinking in real gameplay.** Built and unit-tested 2026-08-30, but every threshold in `POTION_RULES` is an unvalidated starting point — the numbers are a guess at playstyle, not a measurement. Watch the `F5` panel while taking real damage and check: does the ordinary tier fire around 35%, does the emergency tier beat it to the punch on a burst, and does the belt last a reasonable time?
  - `F6` snoozes it for 10s if it misbehaves; `POTION_DRINKING_ENABLED = False` in `main.py` turns it off outright
  - the cooldowns (`health` 4.0s, `rejuvenation` 1.0s) are guesses at how long a D2R heal takes to land — if it double-drinks on one dip, that number is too low
  - `POTION_MIN_GAP_SECONDS` (0.6) is the safety net, not a tuning knob — raise the per-rule cooldowns instead
  - both meter profiles (1280x800 and 1920x1080) are calibrated, so it should fire in either mode
...
- [ ] **An "am I actually in play?" check.** The lobby/menu bug (2026-08-30) was fixed at the sensor — meters now say `None` instead of 0% — but that is the *symptom* layer. The engine still has no notion of "the game is not in a playable state", so the pipeline happily runs detection and OCR against a lobby screen. A template match on a HUD element that only exists during play would give every consumer a cheap, positive signal instead of each one inferring it from missing data. This is the same item as "Town vs. combat detection" below, promoted by having a concrete bug behind it now.
