...
def load_meters(path):
    """Loads meter definitions from JSON. Returns [] (with a warning) if the file is missing or
    ...
    for name, spec in raw.items():
        ...
        try:
            ...
            print(f"WARNING: skipping malformed meter '{name}' in {path}: {error}")

    return meters


def save_meters(path, meters):
    payload = {
        "_comment": (
            "Generated by calibrate_meters.py. 'region' is (x, y, w, h) as fractions of the "
            "full screen, so it stays correct at any capture scale. 'hsv_ranges' are OpenCV "
            "HSV bounds (H 0-179, S 0-255, V 0-255), OR-ed together - red needs two because it "
            "wraps around H=0. Re-run the calibrator after any resolution or UI-scale change."
        )
    }
    for meter in meters:
        payload[meter.name] = {
